...
GooFlow.prototype = {
    ...
    setName: function (id, name, type) {
        ...
        if (type == "node") {//如果是结点
            ...
            if (this.$nodeData[id].type == "start" || this.$nodeData[id].type == "end") {
                this.$nodeDom[id].children(".span").html(name); //.text(name);
            }
            else {
                this.$nodeDom[id].find("td:eq(1)").html(name);  //.text(name);
                var hack = 0;
                if (navigator.userAgent.indexOf("8.0") != -1) hack = 2;
    ...
    markItem: function (id, type, mark) {
        if (type == "node") {
            ...
            var lineDom = this.$lineDom[id];
            if (GooFlow.prototype.useSVG != "") {
                if (mark) {
                    lineDom.childNodes[1].setAttribute("stroke", "#ff3300");
                    lineDom.childNodes[1].setAttribute("marker-end", "url(#arrow2)");
                } else {
                    lineDom.childNodes[1].setAttribute("stroke", "#5068AE");
                    lineDom.childNodes[1].setAttribute("marker-end", "url(#arrow1)");
                }
            } else {
                if (mark) lineDom.strokeColor = "#ff3300";
                else lineDom.strokeColor = "#5068AE"
            }
        }
        if (this.$undoStatck) {
